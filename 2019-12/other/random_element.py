...
# Hope your interview practicing is going well! Let's work through the problem of
#  uniformly picking a random element from a gigantic stream. This is a common
...
# list, find its size, and pick a random element from [0, size - 1]. The problem with
#  this approach is that it would take O(N) space for a large N.

# Instead, let's attempt to solve using loop invariants. On the ith iteration of our
#  loop to pick a random element, let's assume we already picked an element uniformly
#  from [0, i - 1]. In order to maintain the loop invariant, we would need to pick the
#  ith element as the new random element at 1 / (i + 1) chance. For the base case where
